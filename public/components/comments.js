var CommentBox = React.createClass({
    ...
    render: function() {
        return( < div className = "commentBox" > < h1 > Zoover Activity < /h1>
                <br / > < h2 > Here 's what'
            s going on right now. < /h2>
                <CommentList comments={this.state.comments}/ > < CommentForm submitComment = {
                this.submitComment
            }
            />
            </div > );
    ...
    render: function () {
        return (
            <div className="comment">
                <span className="author">{this.props.comment.author}</span > said: < br / > < div className = "body" > {
...
var CommentForm = React.createClass({
    ...
    render: function() {
        return( < form className = "commentForm"
            ...
            ref = "author"
            placeholder = "Name"
            required / > < br / > < textarea name = "text"
            ref = "text"
            placeholder = "Zoover"
            required > < /textarea><br/ > < button type = "submit"
            ref = "submitButton" > Zoov! < /button>
            </form > );
    }
});
