var CommentBox = React.createClass({
    ...
    submitComment: function (comment, callback) {
        this.socket.emit('newComment', comment, function (err) {
            if (err)
                return console.error('Server side error:', err);
            callback();
        });
    },
    render: function() {
    ...
    render: function () {
        return (
            <div className="comment">
                <span className="author">{this.props.comment.author}</span> posted:<br/>
...
var CommentForm = React.createClass({
    handleSubmit: function (e) {
        ...
        submitButton.innerHTML = 'Posting on Zoover...';
